...
bool Sopa_Letras::palabranodescubierta(string p){
  ...
  for(it = ocultas.begin(); it != ocultas.end() && !nodescubierta; ++it)
    if(p == (*it))
      nodescubierta = true;
...
void Sopa_Letras::Poner_Acertada(string p, int f, int c, string direccion){
  ...
  if(Comprobar_Palabra(p, f, c, direccion)){
    for(it = ocultas.begin(); it != ocultas.end(); ++it)
      if((*it) == p){
        encontradas.push(p);
        it = ocultas.erase(it);
    ...
    for(int j = s.sp.cmenor()-1; j < s.sp.cmayor()+1; j++){
      if(j == s.sp.cmenor()-1)
        ...
        if(s.sp.elemento(i, j) == s.sp.valordefecto()){
          ...
          char c = (char)(n+65); // cambia el entero a un caracter en mayusculas aleatorio
          fo << c << "\t";
        }
        else{
          if(s.sp.elemento(i, j) == s.acertadas.elemento(i, j)){
            fo << KGRN << bold_on << s.sp.elemento(i, j) << bold_off << "\t"; // ponemos a negrita y en color verde
          }
          else
